...
function ReceivedTSAResponse(tsaName){
    ...
    if (window._el_timestamps.style.display == "block")
    for (var t = 0; t < _usedTSA.length; t++) {
        if (!_usedTSA[t].input.checked)
            continue;
        if (tsaName === _usedTSA[t].name) {
            _usedTSA[t].replied = true;
            _usedTSA[t].processingStyle.display = "none";
        }
        fReceivedAll = fReceivedAll && _usedTSA[t].replied;
    }
    if (fReceivedAll) {
        _zip.generateAsync({
            type: "arraybuffer",
...
function GenerateTimeSignatures(digest) {
    ...
    for (var t = 0; t < _usedTSA.length; t++) {
        if (!_usedTSA[t].input.checked)
            continue;
        _usedTSA[t].processingStyle.display = "inline";
        var tsaName = _usedTSA[t].name;
        var tsaURL = _usedTSA[t].url;
        // var tsa = _usedTSA[t];
        _usedTSA[t].replied = false;
        if (tsaName === "opentimestamps.org") {
            ...
            //   saveAs(new Blob([fileOts]), "output.ots");
              _zip.file("timestamps/opentimestamps.org.ots", new Blob([fileOts]));
            ...
            if (!blobTsq) {
                ...
                blobTsq = _base64ToArrayBuffer(b64);

                _zip.file("timestamps/request.tsq", blobTsq);
            }
            ...
            xhr.onreadystatechange = function () {
                if (this.readyState === XMLHttpRequest.DONE) {
                    if (this.status >= 200 && this.status <= 299) {
                        ...
                        _zip.file("timestamps/" + this.localTsaName + ".tsr", this.response);
                        ReceivedTSAResponse(this.localTsaName);
                    // upload completed
                    } else {
                        alert(this.localTsaName + ": error " + this.status + ": " + this.statusText);//404 "Not found"
                        ReceivedTSAResponse(this.localTsaName);
                    }
                }
            };
...
function SaveOperationFormToZIP() {
    ...
    // _usedTSA = [];
    // _usedTSA.push({name:"freetsa.org",url:"https://freetsa.org/tsr",replied:false});
    // _usedTSA.push({name:"opentimestamps.org",url:"",replied:false});
    var operationJson = JSON.stringify(operationData, null, 2);
